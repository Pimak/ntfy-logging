...
| `proxy` | String | `system` | How to reach the ntfy server: `system` (or unset) inherits the JVM's default proxy selector, `none` forces a direct connection, `host:port` routes through that proxy (IPv6 bracketed: `[2001:db8::1]:3128`). **The default is not "no proxy"** — a JDK `HttpClient` already honors `-Dhttps.proxyHost` and `-Dhttp.nonProxyHosts`, so `system` is a genuine no-op; set this only to route ntfy differently from the rest of the JVM, or to escape a JVM-wide proxy that cannot reach an internal server (`none`). A malformed value warns and falls back to `system` rather than blocking alerting. See [network.md](network.md). |
| `truststore-path` | String | *(none)* | Path to a trust store holding the CA that signed the ntfy server's certificate — for a self-hosted server behind a private CA or a TLS-inspecting proxy. **Replaces** the default trust anchors for ntfy traffic only (like `javax.net.ssl.trustStore`, but scoped: the JVM-wide switch swaps `cacerts` out from under *every* TLS connection the application makes). Unset uses the JDK default trust material. A store that cannot be loaded refuses activation rather than silently falling back. See [network.md](network.md). |
| `truststore-password` | String | *(none)* | Password for `truststore-path`. Omit for an unprotected store or when `truststore-type` is `PEM`. Never echoed in diagnostics. |
| `truststore-type` | String | `PKCS12` | Format of `truststore-path`: `PKCS12` (default), `JKS`, or `PEM` for a plain certificate file. `PEM` is usually the right choice on Kubernetes, where a corporate CA arrives as a `ca.crt` mounted from a ConfigMap — reading it directly avoids a `keytool -import` step. |
...
- [network.md](network.md) — `proxy` and the `truststore-*` trio in context: what the JVM's own
  proxy/truststore switches already do, when they are not enough, and the `httpClientCustomizer`
  escape hatch.
