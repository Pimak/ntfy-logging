...
/**
 * Clean-shutdown behavior through the example's own {@code log4j2.xml}: stopping the {@link
 * LoggerContext} (what the configured {@code shutdownHook="enable"} does on JVM exit) stops the
 * ntfy appender, which flushes the pending storm digest — and, in async mode, first folds
 * queued-or-in-flight alerts into that digest so nothing is silently lost. No engine thread
 * survives the stop.
 */
class Log4j2CleanShutdownIT {
      ...
      LoggerContext context =
          ExampleLoggerContexts.startFromExampleXml(
              Map.of(
                  "ntfy.url", server.url(),
                  "ntfy.topic", "alerts",
                  // Synchronous: request #0 is then necessarily the allowed alert and #1 the digest.
                  "ntfy.async", "false",
                  "ntfy.maxAlertsPerWindow", "1",
                  // Long window: the digest below can only come from the shutdown flush.
                  "ntfy.suppressionWindow", "60000"));
