...
/**
 * Resolves and vets the ambient ntfy configuration for the programmatic log4j2 entry point ({@link
 * NtfyLog4j2Installer}). Config is loaded via {@link ConfigLoader#load()} (system properties &gt;
 * env &gt; classpath {@code ntfy.properties} &gt; defaults) and then passed through the same
 * supply-chain guard as the Logback and JUL zero-code paths: an endpoint URL supplied
 * <em>only</em> by a classpath {@code ntfy.properties} is refused unless the operator opted in via
 * {@code ntfy.allow-classpath-endpoint} / {@code NTFY_ALLOW_CLASSPATH_ENDPOINT}, because any jar on
 * the classpath can carry such a file.
 *
 * <p>Note this guard applies to the AMBIENT path only. A URL written into an application's own
 * {@code log4j2.xml} as a {@code <Ntfy url="..."/>} attribute is an explicit operator choice and
 * never reaches this class.
 *
 * <p>Split from the entry point so the guard exists exactly once and is testable against a
 * hand-built {@link NtfyConfig} without touching real environment state.
 */
final class Log4j2AmbientConfig {

  private Log4j2AmbientConfig() {}
  ...
  static Optional<NtfyConfig> vet(NtfyConfig cfg, Diagnostics diagnostics) {
    ...
    if (cfg.isEndpointFromClasspathFile()) {
      ...
      // Opted in: still WARN (not info) so the destination lands on the StatusLogger's default
      // console output and an unexpected endpoint is caught immediately.
      diagnostics.warn(
