 ...
 * and a {@code null} body publishes an empty one. Beyond that, a value carrying a character outside
 * printable ASCII is dropped rather than failing the request — one bad configured value must not
 * abort every publish at the header-build boundary. {@code title} is the exception: the publisher
 * RFC 2047 encodes it instead, since a non-ASCII application name is ordinary rather than a
 * mistake.
 *
 * @param title the ntfy {@code Title} header; non-ASCII titles are RFC 2047 encoded by the
 *     publisher
