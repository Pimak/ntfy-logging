...
| `ntfy startup self-test FAILED (<reason>) — the request never received an HTTP response; check url, DNS, TLS/certificate trust, and any egress firewall or proxy` | warn | No HTTP response came back at all: a refused connection, a DNS failure, a timeout — **or a TLS handshake / certificate-trust failure**, since the self-test validates TLS too. `<reason>` is a fixed, type-derived word (`timeout`, `connection refused`, `probe failed: SSLHandshakeException`, …) — never a raw exception message, which could embed a credential. | Read `<reason>` first: an `SSL…Exception` means the certificate chain is not trusted by this JVM, not that the host is unreachable. Otherwise check `url`'s host and port, DNS resolution from inside the container, and outbound firewall/proxy rules. |
...
| `an ntfy cache or firebase value could not be read as a boolean (expected true/false, yes/no, on/off or 1/0) — that setting falls back to its default, and note that an unreadable value in a higher-priority layer also hides any valid value set beneath it` | warn | A `cache` or `firebase` value was supplied but is not a spelling this library recognises. The default stands rather than being guessed at, because guessing wrong here silently costs delivered alerts. The offending value is deliberately not echoed. | Use one of the listed spellings. See [configuration.md](configuration.md#delivery-privacy). |
| `the ntfy icon setting is not a fetchable URL (expected http:// or https:// with a host, written in printable ASCII — percent-encode a unicode path and punycode an international domain) — it is ignored and alerting continues without an icon` | warn | The `icon` value is not something a subscriber's client could fetch. Alerting is unaffected; only the icon is dropped. The offending value is deliberately not echoed. | Give an `http`/`https` URL with a host, in printable ASCII. See [configuration.md](configuration.md#icons). |
| `at least one ntfy icons-by-logger entry was discarded — an entry must read prefix=url and the url must be http:// or https:// with a host; the remaining entries and the default icon are unaffected` | warn | One or more table entries were malformed (no `=`, or a blank half) or carried an unfetchable URL. The rest of the table still works. | Check each `prefix=url` pair. See [configuration.md](configuration.md#icons). |
| `an ntfy icon URL ends in an extension ntfy does not render (only JPEG and PNG are supported) — it is still sent, but the notification will probably show no icon` | warn | An icon URL ends in `.svg`, `.gif` or similar. Nothing is dropped: the format lives in the bytes served, not in the path, so the value is sent anyway. | Point at a JPEG or PNG, or ignore this if the URL genuinely serves one. |
